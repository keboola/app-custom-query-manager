...
WITH (
    ...
    ,FIRSTROW=2
            ...
            "INSERT INTO {{ id(schemaName) }}.{{ id(tableName) }} ([column1], [column2], [_timestamp]) (SELECT CAST(COALESCE([column1], '') as NVARCHAR) AS [column1],CAST(COALESCE([column2], '') as NVARCHAR) AS [column2],{{ timestamp }} FROM {{ id(stageSchemaName) }}.{{ id(tableName ~ rand ~ '_tmp') }} AS [src])",
            "COMMIT",
        ];
